...
import sys
import darkdetect
from game_data import SHIP_TYPES, FLEET_NAME_LIMIT, SHIP_NAME_LIMIT, UNLOCKABLE_SHIPS_ORDER, MUNITION_TYPES, SQUADRON_TYPES
import parse_save
import mod

# ... (Helper function and Constants remain the same) ...
...
LIGHT_THEME = { "bg": "#f0f0f0", "fg": "#000000", "bg_widget": "#ffffff", "fg_select": "#ffffff", "bg_select": "#0078d7" }
DARK_THEME = { "bg": "#2e2e2e", "fg": "#ffffff", "bg_widget": "#3c3c3c", "fg_select": "#ffffff", "bg_select": "#0078d7" }
...
class BSGSaveEditor:
    def __init__(self, root):
        ...
        self._load_config()
        self.setup_menu(); self.setup_ui(); self._update_widget_states()
    ...
    # ... (Most methods are unchanged) ...
    def _load_config(self):
        try:
            if os.path.exists(CONFIG_FILE):
                with open(CONFIG_FILE, 'r') as f: config = json.load(f); self.theme_var.set(config.get("theme", "System"))
        except (IOError, json.JSONDecodeError) as e: print(f"Could not load config file: {e}"); self.theme_var.set("System")
    ...
    def _apply_theme(self, theme_palette):
        bg = theme_palette["bg"]; fg = theme_palette["fg"]; bg_widget = theme_palette["bg_widget"]; bg_select = theme_palette["bg_select"]; fg_select = theme_palette["fg_select"]
        ...
        style.configure('TCheckbutton', background=bg, foreground=fg); style.map('TCheckbutton', background=[('active', bg)], foreground=[('active', fg)])
    ...
    def setup_ui(self):
        ...
        for ship_name in UNLOCKABLE_SHIPS_ORDER:
            ...
            if ship_id is not None:
                ...
                cb = ttk.Checkbutton(unlock_frame, text=ship_name, variable=var)
                cb.grid(row=row, column=col, sticky="w", padx=10, pady=2)
                self.unlock_checkboxes.append(cb); col += 1
                if col >= 3: col = 0; row += 1

        # --- Create Squadron Unlocks Frame ---
        ...
        for squadron_name, squadron_id in sorted(SQUADRON_TYPES.items()):
            if squadron_id in self.unlocked_squadron_vars:
                ...
                cb = ttk.Checkbutton(squadron_unlock_frame, text=squadron_name, variable=var)
                cb.grid(row=row, column=col, sticky="w", padx=10, pady=2)
                self.squadron_checkboxes.append(cb)
                col += 1
                if col >= 2: # 2 columns for a 4x2 grid
                    col = 0
                    row += 1
        ...
        for munition_name, munition_id in sorted(MUNITION_TYPES.items()):
            if munition_id in self.unlocked_munition_vars:
                ...
                cb = ttk.Checkbutton(munition_unlock_frame, text=munition_name, variable=var)
                cb.grid(row=row, column=col, sticky="w", padx=10, pady=2)
                self.munition_checkboxes.append(cb)
                col += 1
                if col >= 3: col = 0; row += 1
    ...
    def _update_widget_states(self):
        ...
        for cb in self.unlock_checkboxes: cb.config(state=tk.NORMAL if file_loaded else tk.DISABLED)
        for cb in self.munition_checkboxes: cb.config(state=tk.NORMAL if file_loaded else tk.DISABLED)
        for cb in self.squadron_checkboxes: cb.config(state=tk.NORMAL if file_loaded else tk.DISABLED)
    ...
    def prompt_for_auto_load(self):
        ...
        self.center_window(top); self.root.wait_visibility(); self.root.wait_window(top)
